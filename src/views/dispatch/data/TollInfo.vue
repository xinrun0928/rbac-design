<template>
  <div class="toll-info-management">
    <el-card class="table-card animate-item" shadow="never">
      <div class="search-bar">
        <span class="search-bar-title">收费站信息</span>
        <div class="search-bar-actions">
          <DataAccessButton text="收费站接入" @click="handleAdd" />
          <ExportButton />
        </div>
      </div>
      ...
      >
        ...
        <el-table-column prop="name" label="收费站名称" min-width="140" align="center" />
...
import { tollInfoData } from '@/mock/dispatch/roadData'
import type { TollInfo } from '@/types/dispatch/roadData'

const tableData = ref<TollInfo[]>([...tollInfoData])
...
const handleAdd = () => {
  ElMessage.info('收费站接入功能开发中...')
}

const handleRemove = (row: TollInfo) => {
  ElMessageBox.confirm(`确定要移除收费站"${row.name}"吗？`, '移除确认', {
    type: 'warning',
