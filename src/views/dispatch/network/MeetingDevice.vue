<template>
  <div class="meeting-device-management">
    <el-card class="table-card animate-item" shadow="never">
      <div class="search-bar">
        <span class="search-bar-title">会议设备管理</span>
...
import { meetingDeviceData } from '@/mock/dispatch/networkData'
import type { MeetingDevice } from '@/types/dispatch/network'

const tableData = ref<MeetingDevice[]>([...meetingDeviceData])
...
const handleStartMeeting = (row: MeetingDevice) => { ElMessage.info(`发起会议：${row.deviceName}`) }
const handleJoinMeeting = (row: MeetingDevice) => { ElMessage.info(`加入会议：${row.deviceName}`) }
const handleDetail = (row: MeetingDevice) => { ElMessage.info(`查看详情：${row.deviceName}`) }

const handleRemove = (row: MeetingDevice) => {
