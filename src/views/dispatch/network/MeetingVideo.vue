<template>
  <div class="meeting-video-management">
    <el-card class="table-card animate-item" shadow="never">
      <div class="search-bar">
        <span class="search-bar-title">会议视频</span>
...
import { meetingVideoData } from '@/mock/dispatch/networkData'
import type { MeetingVideo } from '@/types/dispatch/network'

const tableData = ref<MeetingVideo[]>([...meetingVideoData])
...
const handleStartMeeting = (row: MeetingVideo) => { ElMessage.info(`发起会议：${row.deviceName}`) }
const handleJoinMeeting = (row: MeetingVideo) => { ElMessage.info(`加入会议：${row.deviceName}`) }
const handleDetail = (row: MeetingVideo) => { ElMessage.info(`查看详情：${row.deviceName}`) }

const handleRemove = (row: MeetingVideo) => {
