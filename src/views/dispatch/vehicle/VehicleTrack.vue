  ...
  <div class="vehicle-track">
    <el-card class="table-card animate-item" shadow="never">
      <div class="search-bar">
        <span class="search-bar-title">重点车辆行驶轨迹</span>
        <div class="search-bar-actions">
          <DataAccessButton text="重点车辆接入" @click="handleAdd" />
      ...
      >
        ...
        <el-table-column prop="driveTime" label="行驶时间" width="150" align="center" />
        <el-table-column label="行驶状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag type="warning" effect="plain" size="small">{{ row.driveStatus }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="速度 (km/h)" width="110" align="center">
          <template #default="{ row }">
            {{ row.speed }}
          </template>
        </el-table-column>
        <el-table-column prop="recordInfo" label="记录信息" min-width="120" align="center" show-overflow-tooltip />
        <el-table-column label="操作" width="200" align="center" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" link size="small" @click="handleInputTrack(row)">车辆轨迹录入</el-button>
...
import { vehicleTrackData } from '@/mock/dispatch/meetingData'
import type { VehicleTrack } from '@/types/dispatch/meeting'

const tableData = ref<VehicleTrack[]>([...vehicleTrackData])
...
const handleAdd = () => { ElMessage.info('重点车辆接入功能开发中...') }

const handleInputTrack = (row: VehicleTrack) => { ElMessage.info(`车辆轨迹录入：${row.plateNumber}`) }
const handleDetail = (row: VehicleTrack) => { ElMessage.info(`查看详情：${row.plateNumber}`) }

const handleRemove = (row: VehicleTrack) => {
  ElMessageBox.confirm(`确定要移除车辆"${row.plateNumber}"吗？`, '移除确认', {
